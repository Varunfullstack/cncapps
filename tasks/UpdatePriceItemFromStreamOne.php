            ...
            foreach($order["lines"] as $line)
            {
                if(isset($line["addOns"])&&$line["lineStatus"]=='active')
                {
                    foreach($line["addOns"] as $addon)
                    {
                        ...
                        array_push($allAddons,(object)["orderNumber"=>$order["orderNumber"],"email"=>$order["endUserEmail"],"sku"=>$addon["sku"],"quantity"=>$addon["quantity"],"addOnStatus"=>$addon["addOnStatus"]]);
                    }
                }
            }
    }
   ...
   // echo "addons Count".count($allAddons)."\n";    
    ...
    foreach($allAddons as $addon)
    {
     ...
     foreach($items as $addOn)
     {
            ...
            {
                $db->query("select cui_users quantity,cui_cuino from custitem 
                where   
                    renewalStatus='R'  
                    and declinedFlag='N'
                ...
                //  echo " ".$temp[0]["quantity"];
                    if((int)$quantity!=(int)$temp[0]["quantity"]&&$status=="active")
                    {
                        ...
                        //echo  $customer["id"]." ".$itemId." ".(int)$subscription->quantity." ".(int)$temp[0]["quantity"]."\n";
                        $db->query("update custitem set cui_users=$quantity 
                                    where   
                                    cui_cuino=$cui_cuino");

  ...
  */
  ...
  function displayTeamPerformanceReport()
  {
    ...
    foreach ( $results as $result ){
      ...
      if( round( $result[ 'esTeamActualSlaPercentage' ], 2 ) < $result[ 'esTeamTargetSlaPercentage' ] ){
        
        $this->template->set_var( 'esTeamActualSlaPercentage' . $result[ 'quarter' ] . 'Class', 'performance-warn' );
      }

      if( round( $result[ 'hdTeamActualSlaPercentage' ], 2 ) < $result[ 'hdTeamTargetSlaPercentage' ] ){
        
        $this->template->set_var( 'hdTeamActualSlaPercentage' . $result[ 'quarter' ] . 'Class', 'performance-warn' );
      }

      if( round( $result[ 'imTeamActualSlaPercentage' ], 2 ) < $result[ 'imTeamTargetSlaPercentage' ] ){
      ...
      if( round( $result[ 'esTeamActualFixHours' ], 2 ) > $result[ 'esTeamTargetFixHours' ] ){
        
        $this->template->set_var( 'esTeamActualFixHours' . $result[ 'quarter' ] . 'Class', 'performance-warn' );
      }

      if( round( $result[ 'hdTeamActualFixHours' ],2 ) > $result[ 'hdTeamTargetFixHours' ] ){
        
        $this->template->set_var( 'hdTeamActualFixHours' . $result[ 'quarter' ] . 'Class', 'performance-warn' );
      }

      if( round( $result[ 'imTeamActualFixHours' ], 2 ) > $result[ 'imTeamTargetFixHours' ] ){

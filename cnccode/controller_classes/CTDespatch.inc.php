...
// Messages
define('CTDESPATCH_MSG_PURCHASEORDER_NOT_FND', 'Purchase Order not found');
define('CTDESPATCH_MSG_ORDHEADID_NOT_PASSED', 'ordheadID not passed');
define('CTDESPATCH_MSG_DELIVERYNOTEID_NOT_PASSED', 'deliveryNoteID not passed');
define('CTDESPATCH_MSG_SUPPLIERID_NOT_PASSED', 'supplierID not passed');
define('CTDESPATCH_MSG_MUST_BE_INITIAL', 'Must be an initial order');
define('CTDESPATCH_MSG_SEQNO_NOT_PASSED', 'sequence no not passed');
define('CTDESPATCH_MSG_ORDLINE_NOT_FND', 'order line not found');
// Actions
define('CTDESPATCH_ACT_DISP_SEARCH', 'dispSearch');
define('CTDESPATCH_ACT_DESPATCH', 'despatch');
// HTML CSS classes
define('CTDESPATCH_CLS_ORDER_LINE_ITEM', 'orderLineItem');
define('CTDESPATCH_CLS_ORDER_LINE_COMMENT', 'orderLineComment');
class CTDespatch extends CTCNC {
	...
	function CTDespatch($requestMethod,	$postVars, $getVars, $cookieVars, $cfg){
	...
	*/
	function displaySearchForm()
	{
		...
		$this->setTemplateFiles	('DespatchSearch', 'DespatchSearch.inc');
// Parameters
		...
		if($this->dsOrdhead->rowCount()>0){
			$this->template->set_block('DespatchSearch','orderBlock', 'orders');
...
// search parameter section
		...
		$this->template->parse('CONTENTS', 	'DespatchSearch', true);
		$this->parsePage();
	}
	/**
	* Display the results of order search
	...
	*/
	function displayDespatch()
	{
		...
		$buSalesOrder->getOrdheadByID($_REQUEST['ordheadID'], $dsOrdhead);
		$dsOrdhead->fetchNext();
		$buDespatch->getLinesByID($dsOrdhead->getValue('ordheadID'), $dsOrdline);
		if (!$this->getFormError()) { 
			$buDespatch->getInitialDespatchQtys($dsOrdline, $this->dsDespatch);
		}
		...
		if ( $buDespatch->countNonReceievedPOsByOrdheadID($ordheadID) > 0 ){
			$this->template->set_var('poNotRecd', 'WARNING: Not all of the purchase orders have been receieved');
		}			
...
//				'renewalsButton' => $renewalsButton
		...
		$this->template->set_block('DespatchDisplay','deliveryMethodBlock', 'deliveryMethods');
		while ($dsDeliveryMethod->fetchNext()){
			$this->template->set_var(
				array(
					...
					'deliveryMethodSelected' => ($this->getDeliveryMethodID() == $dsDeliveryMethod->getValue('deliveryMethodID')) ? CT_SELECTED : ''
				)
			);
			$this->template->parse('deliveryMethods', 'deliveryMethodBlock', true);
		}
    ...
    $this->template->set_var(
      array(
		...
		if ($dsOrdline->rowCount() > 0){
			$this->template->set_block('DespatchDisplay','orderLineBlock', 'orderLines');
		...
		$ctDeliveryNotes = new CTDeliveryNotes($this, $ordheadID, $buDespatch);
		$ctDeliveryNotes->execute();

		$this->template->parse('CONTENTS', 	'DespatchDisplay', true);
		$this->parsePage();
	}
	/**
	* Perform despatch
 ...
 */
	...
	*/
	function displayNoteDoc(){
		...
		$buDespatch = new BUDespatch($this);
		$buDespatch->getDeliveryNoteByID($_REQUEST['deliveryNoteID'], $dsDeliveryNote);

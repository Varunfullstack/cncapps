...
class CTProjectOptions extends CTCNC
{
        ...
        switch ($this->getAction()) {
            case self::CONS_PROJECT_STAGES:
                if ($method == 'GET') echo json_encode(
                    ...
                    JSON_NUMERIC_CHECK
                ); else if ($method == 'POST') echo json_encode(
                    $this->addProjectStage()
                ); else if ($method == 'DELETE') echo json_encode(
                    $this->deleteProjectStage()
                ); else if ($method == 'PUT') echo json_encode($this->updateProjectStage());
                break;
            case self::CONS_PROJECT_TYPES:
                ...
                ); else if ($method == 'PUT') echo json_encode($this->updateProjectType());
                break;
            default:
                $this->setTemplate();
                break;
    ...
    function addProjectStage()
    {
        ...
        if ($body->name != '') {
            ...
            $status     = DBConnect::execute(
                " insert into projectstages(name,stageOrder,displayInSR) values(:name,:stageOrder,:displayInSR)",
    ...
    function updateProjectStage()
    {
        ...
        if (!$type) throw new Exception("Not found", 404);
        return [
            "status" => DBConnect::execute(
                "update projectstages set displayInSR=:displayInSR, name=:name,stageOrder=:stageOrder where id=:id",
                [
                    "id"          => $id,
                    "name"        => $body->name,
                    "stageOrder"  => $body->stageOrder,
                    "displayInSR" => $body->displayInSR
                ]
            )
        ];
    }
    ...
    function deleteProjectStage()
    {
        ...
        return DBConnect::execute(
            "delete from projectstages  where id=:id",
            ["id" => $id]
        );
    }
    //------------------End ProjectStages
    ...
    function addProjectType()
    {
        ...
        if ($body->name != '') return [
            "status" => DBConnect::execute(
                " insert into ProjectTypes(name,includeInWeeklyReport,notes) values(:name,:includeInWeeklyReport,:notes)",
                [
                    "name"                  => $body->name,
                    "includeInWeeklyReport" => $body->includeInWeeklyReport,
                    "notes"                 => $body->notes
                ]
            )
        ]; else return ["status" => false];
    ...
    function updateProjectType()
    {
        ...
        return [
            "status" => DBConnect::execute(
                "update ProjectTypes set name=:name,includeInWeeklyReport=:includeInWeeklyReport,notes=:notes where id=:id",
                [
                    "id"                    => $id,
                    "name"                  => $body->name,
                    "includeInWeeklyReport" => $body->includeInWeeklyReport,
    ...
    function deleteProjectType()
    {
        ...
        if (!$type) throw new Exception("Not found", 404);
        return DBConnect::execute(
            "delete from ProjectTypes  where id=:id",
            ["id" => $id]
        );
    }
    //------------------End ProjectTypes

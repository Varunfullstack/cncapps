            ...
            if (!$dsSearchForm->populateFromArray($_REQUEST ['searchForm'])) {
                ...
                foreach ($results as $result) {
                    ...
                    if ($result['esTeamActualSlaPercentage'] < $result['esTeamTargetSlaPercentage']) {
                        ...
                        $this->template->set_var('esTeamActualSlaPercentage' . $result['month'] . 'Class',
                                                 'performance-warn');
                    }

                    if ($result['hdTeamActualSlaPercentage'] < $result['hdTeamTargetSlaPercentage']) {

                        $this->template->set_var('hdTeamActualSlaPercentage' . $result['month'] . 'Class',
                                                 'performance-warn');
                    }

                    if ($result['imTeamActualSlaPercentage'] < $result['imTeamTargetSlaPercentage']) {

                        $this->template->set_var('imTeamActualSlaPercentage' . $result['month'] . 'Class',
                                                 'performance-warn');
                    }

                    if ($result['esTeamActualFixQtyPerMonth'] < $result['esTeamTargetFixQtyPerMonth']) {

                        $this->template->set_var('esTeamActualFixQtyPerMonth' . $result['month'] . 'Class',
                                                 'performance-warn');
                    }

                    if ($result['hdTeamActualFixQtyPerMonth'] < $result['hdTeamTargetFixQtyPerMonth']) {

                        $this->template->set_var('hdTeamActualFixQtyPerMonth' . $result['month'] . 'Class',
                                                 'performance-warn');
                    }

                    if ($result['imTeamActualFixQtyPerMonth'] < $result['imTeamTargetFixQtyPerMonth']) {

                        $this->template->set_var('imTeamActualFixQtyPerMonth' . $result['month'] . 'Class',
                                                 'performance-warn');
                    }

                    if ($result['esTeamActualFixHours'] > $result['esTeamTargetFixHours']) {

                        $this->template->set_var('esTeamActualFixHours' . $result['month'] . 'Class',
                                                 'performance-warn');
                    }
                    if ($result['hdTeamActualFixHours'] > $result['hdTeamTargetFixHours']) {

                        $this->template->set_var('hdTeamActualFixHours' . $result['month'] . 'Class',
                                                 'performance-warn');
                    }

                    if ($result['imTeamActualFixHours'] > $result['imTeamTargetFixHours']) {

                        $this->template->set_var('imTeamActualFixHours' . $result['month'] . 'Class',
                                                 'performance-warn');
                    }

                    $this->template->set_var(
                        array(
                            'esTeamActualSlaPercentage' . $result['month'] => number_format($result['esTeamActualSlaPercentage'],
                                                                                            0),
                            ...
                            'imTeamActualSlaPercentage' . $result['month'] => number_format($result['imTeamActualSlaPercentage'],
                                                                                            0),
                            ...
                            'hdTeamActualSlaPercentage' . $result['month'] => number_format($result['hdTeamActualSlaPercentage'],
                                                                                            0),

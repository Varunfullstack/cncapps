...
class CTActivityProfitReport extends CTCNC
{
    ...
    function displaySearchForm()
    {
        ...
        while ($this->dsResults->fetchNext()) {
            ...
            $this->template->set_var(
                array(
                    ...
                    'SDHours'          => number_format(
                        $this->dsResults->getValue('SDHours'),
                        2
                    ),
                    'SDCharge'         => number_format(
                        $this->dsResults->getValue('SDCharge'),
                        2
                    ),
                    'SDProRata'        => number_format(
                        $this->dsResults->getValue('SDProRata'),
                        2
                    ),
                    'PPHours'          => number_format(
                        $this->dsResults->getValue('PPHours'),
                        2
                    ),
                    'PPCharge'         => number_format(
                        $this->dsResults->getValue('PPCharge'),
                        2
                    ),
                    'SCHours'          => number_format(
                        $this->dsResults->getValue('SCHours'),
                        2
                    ),
                    'SCCharge'         => number_format(
                        $this->dsResults->getValue('SCCharge'),
                        2
                    ),
                    'SCProRata'        => number_format(
                        $this->dsResults->getValue('SCProRata'),
                        2
                    ),
                    'TMHours'          => number_format(
                        $this->dsResults->getValue('TMHours'),
                        2
                    ),
                    'TMCharge'         => number_format(
                        $this->dsResults->getValue('TMCharge'),
                        2
                    ),
                    'TotalHours'       => number_format(
                        $this->dsResults->getValue('TotalHours'),
                        0
                    ),
                    'CncCost'          => number_format(
                        $this->dsResults->getValue('CncCost'),
                        2
                    ),
                    'SalesProfit'      => number_format(
                        $this->dsResults->getValue('SalesProfit'),
                        2
                    ),
                    'ServiceProfit'    => number_format(
                        $this->dsResults->getValue('ServiceProfit'),
                        2
                    ),
                    'urlDrill'         => $urlDrill,
                    'urlDrillInvoices' => $urlDrillInvoices
                )
            );
        ...
        $this->template->set_var(
            array(
                ...
                'urlSubmit'          => $urlSubmit,
                'SDHoursTotal'       => number_format(
                    $SDHoursTotal,
                    2
                ),
                'SDChargeTotal'      => number_format(
                    $SDChargeTotal,
                    2
                ),
                'SDProRataTotal'     => number_format(
                    $SDProRataTotal,
                    2
                ),
                'PPHoursTotal'       => number_format(
                    $PPHoursTotal,
                    2
                ),
                'PPChargeTotal'      => number_format(
                    $PPChargeTotal,
                    2
                ),
                'SCHoursTotal'       => number_format(
                    $SCHoursTotal,
                    2
                ),
                'SCChargeTotal'      => number_format(
                    $SCChargeTotal,
                    2
                ),
                'SCProRataTotal'     => number_format(
                    $SCProRataTotal,
                    2
                ),
                'TMHoursTotal'       => number_format(
                    $TMHoursTotal,
                    2
                ),
                'TMChargeTotal'      => number_format(
                    $TMChargeTotal,
                    2
                ),
                'TotalHoursTotal'    => number_format(
                    $TotalHoursTotal,
                    2
                ),
                'CncCostTotal'       => number_format(
                    $CncCostTotal,
                    2
                ),
                'SalesProfitTotal'   => number_format(
                    $SalesProfitTotal,
                    2
                ),
                'ServiceProfitTotal' => number_format(
                    $ServiceProfitTotal,
                    2
                )
            )
    ...
    function drillDown()
    {
        ...
        $dsResults = $this->buActivityProfitReport->searchDrill(
            $_REQUEST ['customerID'],
            $_REQUEST ['fromDate'],
            $_REQUEST ['toDate']
        );
        ...
        while ($dsResults->fetchNext()) {
            ...
            $this->template->set_var(
                array(
                    'ActivityID' => $dsResults->getValue('ActivityID'),
                    'Date'       => $dsResults->getValue('Date'),
                    'SDHours'    => number_format(
                        $dsResults->getValue('SDHours'),
                        2
                    ),
                    'SDCharge'   => number_format(
                        $dsResults->getValue('SDCharge'),
                        2
                    ),
                    'PPHours'    => number_format(
                        $dsResults->getValue('PPHours'),
                        2
                    ),
                    'PPCharge'   => number_format(
                        $dsResults->getValue('PPCharge'),
                        2
                    ),
                    'SCHours'    => number_format(
                        $dsResults->getValue('SCHours'),
                        2
                    ),
                    'SCCharge'   => number_format(
                        $dsResults->getValue('SCCharge'),
                        2
                    ),
                    'TMHours'    => number_format(
                        $dsResults->getValue('TMHours'),
                        2
                    ),
                    'TMCharge'   => number_format(
                        $dsResults->getValue('TMCharge'),
                        2
                    )
                )
            );
    ...
    function drillDownInvoices()
    {
        ...
        $this->template->set_var(
            array(
                'ProductSalesTurnover'          => number_format(
                    $results ['ProductSalesTurnover'],
                    2
                ),
                'ProductSalesProfit'            => number_format(
                    $results ['ProductSalesProfit'],
                    2
                ),
                'InternetRevenueTurnover'       => number_format(
                    $results ['InternetRevenueTurnover'],
                    2
                ),
                'InternetRevenueProfit'         => number_format(
                    $results ['InternetRevenueProfit'],
                    2
                ),
                'ManagedServiceRevenueTurnover' => number_format(
                    $results ['ManagedServiceRevenueTurnover'],
                    2
                ),
                'ManagedServiceRevenueProfit'   => number_format(
                    $results ['ManagedServiceRevenueProfit'],
                    2
                ),
                'MaintAndGenSupportTurnover'    => number_format(
                    $results ['MaintAndGenSupportTurnover'],
                    2
                ),
                'MaintAndGenSupportProfit'      => number_format(
                    $results ['MaintAndGenSupportProfit'],
                    2
                )
            )
        );
        ...
        $this->template->parse(
            'CONTENTS',
            'ActivityProfitReportInvoiceDrill',
            true
        );
        $this->parsePage();

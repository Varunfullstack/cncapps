...
class CTKeywordMatchingIgnores extends CTCNC
{
    ...
    function addWord(){
        ...
        if($body->word)
        {
            if($this->isDublicate($body->word))                
                return $this->fail(APIException::conflict,"Word exist.");
            return $this->success(DBConnect::execute("insert into keywordMatchingIgnores(word) values(:word)",["word"=>$body->word]));
        }        
        else
    ...
    function updateWord(){
        ...
        {
            if($this->isDublicate($body->word,$body->id))                
                return $this->fail(APIException::conflict,"Word exist.");
            return $this->success(DBConnect::execute("update keywordMatchingIgnores set word=:word where id=:id",["word"=>$body->word,"id"=>$body->id]));
        }
        else
        return false;
    }
    ...
    function deleteWord(){
        $id=@$_REQUEST["id"];
        if($id)
        return DBConnect::execute("delete from  keywordMatchingIgnores where id=:id",["id"=>$id]);
        else
        return false;
    }
    ...
    function isDublicate($word,$id=null){
        if($id==null)
        {
            return DBConnect::fetchOne("select count(*) total from keywordMatchingIgnores where word=:word",["word"=>$word])["total"]>0;
        }
        else
            return DBConnect::fetchOne("select count(*) total from keywordMatchingIgnores where word=:word and id<>:id",["word"=>$word,"id"=>$id])["total"]>0;
    }
}

...
class DBEPurchaseInv extends DBEntity
{
    ...
    function countRowsBySupplierInvNo($accRef, $ref)
    {
        ...
        $this->setQueryString(
            ...
            " WHERE " . $this->getDBColumnName('accRef') . "=" . mysqli_real_escape_string($this->db->link_id(), $accRef) .
            " AND " . $this->getDBColumnName('ref') . "='" . mysqli_real_escape_string($this->db->link_id(), $ref) . "'"
        );
        if ($this->runQuery()) {
            if ($this->nextRecord()) {

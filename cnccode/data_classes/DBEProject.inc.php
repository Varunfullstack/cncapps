...
class DBEProject extends DBEntity
{
    ...
    function getRowsByCustomerID(
        ...
        $queryString =
            ...
            " LEFT JOIN  `projectstages` ps ON ps.id=project.`projectStageID` WHERE " . $this->getDBColumnName(self::customerID) . "='" . mysqli_real_escape_string(
                $this->db->link_id(),
                $customerID
            ) . "' AND description != ''";
        ...
        if ($activeOnly) {
            $queryString .= " and (project.`projectStageID` IS  NULL OR ps.displayInSr=1) and (expiryDate >= current_date or expiryDate is null)";
        }

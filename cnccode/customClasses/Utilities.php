...
class Utilities
{
    ...
    )
    {
        ...
        if($postParams){ curl_setopt($c, CURLOPT_POST,TRUE);  curl_setopt($c, CURLOPT_POSTFIELDS, (is_array($postParams)? http_build_query($postParams) : $postParams) ); }
        ...
        if(!empty($extra['curlOpts'])) foreach($extra['curlOpts'] as $key=>$value) curl_setopt($c, constant($key), $value);
        $data = curl_exec($c);
        ...
        //URLS correction
        if(function_exists('url_corrections_for_content_HELPER')){	    $data= url_corrections_for_content_HELPER($data, $status['url'],   array('schemeless'=>!empty($extra['schemeless']), 'replaceSrc'=>!empty($extra['replaceSrc']), 'rawgit_replace'=>!empty($extra['rawgit_replace']) )  );    	}
        $answer = ( !empty($extra['returnArray']) ? array('data'=>$data, 'header'=>$header, 'info'=>$status) : $data);
        return $answer;      }     function url_corrections_for_content_HELPER( $content=false, $url=false, 	$extra_opts=array('schemeless'=>false, 'replaceSrc'=>false, 'rawgit_replace'=>false) ) {

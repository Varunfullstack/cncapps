...
class BUPDFSalesQuote extends Business{
  ...
  * @access Public
  */
  function BUPDFSalesQuote(&$owner){
...
*/
  function generate(
    ...
    $this->buSalesOrder->getUserByID( $userID, $dsUser);
    ...
    while ($dsOrdline->fetchNext()){
      if (
        $dsSelectedOrderLine && 
        $dsSelectedOrderLine->search('sequenceNo', $dsOrdline->getValue("sequenceNo"))
      ){
...
*/
    ...
    $buPDF->CR();
    $buPDF->CR();
    $buPDF->printString('If you would like to proceed with this quotation then please forward your written (fax or e-mail) order to us at your earliest convenience.');
    $buPDF->CR();
    $buPDF->CR();
    ...
    if ($dsUser->getValue('signatureFilename')!='') {
      $buPDF->placeImageAt(IMAGES_DIR.'/'.$dsUser->getValue('signatureFilename'), 'PNG', 10, 35);
    }
    $buPDF->CR();
    $buPDF->CR();
    $buPDF->CR();
    ...
    $buPDF->CR();
    $buPDF->placeImageAt( $GLOBALS['cfg']['cncaddress_path'], 'PNG', 6, 200);
    $buPDF->endPage();
    // End of First page 
    ...
    $dsQuotation->setValue('versionNo', $versionNo);
    $dsQuotation->setValue('ordheadID', $dsOrdhead->getValue('ordheadID') );
    $dsQuotation->setValue('userID', $userID);
    $dsQuotation->setValue('sentDateTime', date('0000-00-00 00:00:00'));
    $dsQuotation->setValue('salutation', $salutation );
    $dsQuotation->setValue('emailSubject', $emailSubject);
    $dsQuotation->setValue('fileExtension', 'pdf');
    $dsQuotation->setValue('documentType', 'quotation');
    $dsQuotation->post();
  ...
  */
  function sendPDFEmailQuote( $quotationID )
  {
    ...
    $quoteFile = 'quotes/'.$dbeQuotation->getValue('ordheadID').'_'.$dbeQuotation->getValue('versionNo').'.pdf';

    $body = $dbeQuotation->getValue('documentType') . ' ' . $dsOrdhead->getValue('ordheadID').'/'.$dbeQuotation->getValue('versionNo'). ' for ' . $dbeQuotation->getValue('emailSubject');

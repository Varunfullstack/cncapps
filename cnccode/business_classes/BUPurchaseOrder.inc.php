...
class BUPurchaseOrder extends Business{
	...
	* @access Public
	*/
	function BUPurchaseOrder(&$owner){
	...
	function createPOsFromSO($ordheadID, $userID){
		...
		if (!$buSalesOrder->getOrdheadByID($ordheadID, $dsOrdhead)){
		...
		$buSalesOrder->getOrderItemsForPO($ordheadID, $dsOrdline);
		...
		while ($dsOrdline->fetchNext()){
			...
			if ($dsOrdline->getValue('supplierID') != $lastSupplierID){
				$buSupplier->getSupplierByID($dsOrdline->getValue('supplierID'), $dsSupplier);
				$this->counter = 1;
	...
	*/
	function createNewPO($ordheadID, $supplierID, $userID){
		$dsOrdhead = & $this->dsOrdhead;
		$dsOrdline = & $this->dsOrdline;
		$dsSupplier = & $this->dsSupplier;
	...
	function insertPOHeader($userID){
		...
		$dbePorhead->setValue('ordheadID', $dsOrdhead->getValue('ordheadID'));
		$dbePorhead->setValue('date', date('Y-m-d'));
		$dbePorhead->setValue('supplierID', $dsSupplier->getValue('supplierID'));
		$dbePorhead->setValue('userID', $userID);
	...
	function insertPOLine(){
		...
		$dbePorline->setValue('porheadID', $this->dbePorhead->getValue('porheadID'));	
		$dbePorline->setValue('sequenceNo', $this->counter);	
		$dbePorline->setValue('itemID', $dsOrdline->getValue('itemID'));	
		$dbePorline->setValue('qtyOrdered', $dsOrdline->getValue('qtyOrdered'));	
		$dbePorline->setValue('qtyReceived', 0);	
		$dbePorline->setValue('qtyInvoiced', 0);	
		$dbePorline->setValue('curUnitCost', $dsOrdline->getValue('curUnitCost'));	
		$dbePorline->setValue('stockcat', $dsOrdline->getValue('stockcat'));	
		$dbePorline->setValue('expectedDate', date('Y-m-d', strtotime('+ 3 days'))); // today + 3 days
	...
	function search(
		...
		if ($porheadID!=''){
			$ret=($this->getDatasetByPK($porheadID, $dbeJPorhead, $dsResults));
		}
		else{
	...
	function getOrderByID($porheadID, &$dsPorhead, &$dsPorline){
		if ($porheadID==''){
			$this->raiseError('porheadID not passed');
		}
		$dbeJPorhead=new DBEJPorhead($this);
		if (!$this->getDatasetByPK($porheadID, $dbeJPorhead, $dsPorhead)){
			$this->raiseError('Order not found');
		}
		$dbeJPorline=new DBEJPorline($this);
		$dbeJPorline->setValue('porheadID', $porheadID);
		$dbeJPorline->getRowsByColumn('porheadID');
		$this->getData($dbeJPorline, $dsPorline);
		return TRUE;
	}
	function getOrderHeaderByID($porheadID, &$dsPorhead){
		$this->setMethodName('getOrderHeaderByID');
		if ($porheadID==''){
			$this->raiseError('porheadID not passed');
		}
		$dbeJPorhead=new DBEJPorhead($this);
		return ($this->getDatasetByPK($porheadID, $dbeJPorhead, $dsPorhead));
	}
	function getLinesByID($porheadID, &$dsPorline){
...
*/
	function getOrdlineByIDSeqNo($porheadID, $sequenceNo, &$dsPorline){
		...
		$dbeJPorline=new DBEJPorline($this);
		$dbeJPorline->setValue('porheadID', $porheadID);
		$dbeJPorline->setValue('sequenceNo', $sequenceNo);
		$dbeJPorline->getRowByPorheadIDSequenceNo();
		return ($this->getData($dbeJPorline, $dsPorline));
	}
	/**
	...
	* @access public
	*/
	function initialiseNewOrdline($porheadID, $sequenceNo, &$dsPorline){
		...
		$dsPorline->setValue('porheadID', $porheadID);
		$dsPorline->setValue('itemID', '');
		$dsPorline->setValue('sequenceNo', $sequenceNo);
		$dsPorline->setValue('qtyOrdered', 1);	// default 1
		$dsPorline->setValue('qtyReceived', 0);
		$dsPorline->setValue('qtyInvoiced', 0);
		$dsPorline->setValue('curUnitCost', 0);
		$dsPorline->setValue('expectedDate', date('Y-m-d', strtotime('+ 3 days'))); // today + 3 days
		$dsPorline->post();
	}
	/**
...
//count rows
		...
		$dbePorline->setValue('porheadID', $dsPorline->getValue('porheadID'));
		if ($dbePorline->countRowsByColumn('porheadID') > 0){
			// shuffle down existing rows before inserting new one
			$dbePorline->setValue('porheadID', $dsPorline->getValue('porheadID'));
			$dbePorline->setValue('sequenceNo', $dsPorline->getValue('sequenceNo'));
			$dbePorline->shuffleRowsDown();
		}
		$ret = ($this->updateOrderLine($dsPorline, "I"));
	}
	/* Update order line
	...
	* @access public
	*/
	function updateOrderLine(&$dsPorline, $action = "U"){
		...
		// ordline fields
		$dbePorline=new DBEPorline($this);
		$dbePorline->setValue('qtyOrdered', $dsPorline->getValue('qtyOrdered'));
		$dbePorline->setValue('curUnitCost', $dsPorline->getValue('curUnitCost'));
		$dbePorline->setValue('porheadID', $dsPorline->getValue('porheadID'));
		$dbePorline->setValue('sequenceNo', $dsPorline->getValue('sequenceNo'));
		$dbePorline->setValue('qtyInvoiced', 0);
		$dbePorline->setValue('qtyReceived', 0);
		$dbePorline->setValue('itemID', $dsPorline->getValue('itemID'));
		$dbePorline->setValue('expectedDate', $dsPorline->getValue('expectedDate'));
		$buItem=new BUItem($this);
		if ($buItem->getItemByID($dsPorline->getValue('itemID'), $dsItem)){
			$dbePorline->setValue('stockcat', $dsItem->getValue('stockcat'));
		}
		if ($action == "U"){
	...
	function moveOrderLineUp($porheadID, $sequenceNo){
		if ($porheadID==''){
			$this->raiseError('porheadID not passed');
		}
		if ($sequenceNo==''){
		...
		$dbePorline=new DBEPorline($this);
		$dbePorline->setValue('porheadID', $porheadID);
		$dbePorline->setValue('sequenceNo', $sequenceNo);
		$dbePorline->moveRow('UP');
	}
	function moveOrderLineDown($porheadID, $sequenceNo){
		if ($porheadID==''){
			$this->raiseError('porheadID not passed');
		}
		if ($sequenceNo==''){
	...
	function deleteOrderLine($porheadID, $sequenceNo){
		if ($porheadID==''){
			$this->raiseError('porheadID not passed');
		}
		if ($sequenceNo==''){
	...
	function updateHeader($dsPorhead){
		...
		if ($dsPorhead->getValue('directDeliveryFlag')==''){
			...
			$dsPorhead->setValue('directDeliveryFlag', 'N');
			$dsPorhead->post();
		}
		$this->dbePorhead->getRow($dsPorhead->getValue('porheadID')); //existing values
		$this->dbePorhead->setValue('supplierID', $dsPorhead->getValue('supplierID'));
		$this->dbePorhead->setValue('contactID', $dsPorhead->getValue('contactID'));
		$this->dbePorhead->setValue('supplierRef', $dsPorhead->getValue('supplierRef'));
		$this->dbePorhead->setValue('ordheadID', $dsPorhead->getValue('ordheadID'));
		$this->dbePorhead->setValue('payMethodID', $dsPorhead->getValue('payMethodID'));
		$this->dbePorhead->setValue('directDeliveryFlag', $dsPorhead->getValue('directDeliveryFlag'));
	...
	function setOrderedFields($porheadID, $userID){
		$this->setMethodName('setOrderedFields');
		$this->dbePorhead->getRow($porheadID);
		$this->dbePorhead->setValue('orderUserID', $userID);
		$this->dbePorhead->setValue('orderDate', date('Y-m-d'));
		$this->dbePorhead->updateRow();
		return TRUE;
	}
	/**
	...
	*/
	function getHeaderByID($porheadID, &$dsResults){
		$this->setMethodName('getHeaderByID');
		if ($porheadID==''){
			$this->raiseError('porheadID not passed');
		}
		return $this->getDatasetByPK($porheadID, $this->dbeJPorhead, $dsResults);
	}
	/**
	...
	*/
	function getInitialReceieveQtys(&$dsPorline, &$dsReceive, $addCustomerItems){
		...
		while ($dsPorline->fetchNext()){
			...
			if ( ($dbeItem->getValue('serialNoFlag') == 'Y') & $addCustomerItems )
			{
				for ( $i=1 ; $i <= $qtyOS ; $i++ ){
					...
					$dsReceive->setValue('requireSerialNo', TRUE);
					$dsReceive->setValue('serialNo', '');
					$dsReceive->setValue('description', $dsPorline->getValue('itemDescription'));
					$dsReceive->setValue('sequenceNo', $sequenceNo);
					$dsReceive->setValue('orderSequenceNo', $dsPorline->getValue('sequenceNo'));
					$dsReceive->setValue('qtyOrdered', 1);
					$dsReceive->setValue('qtyReceived', 0);
					$dsReceive->setValue('qtyToReceive', 0);
					$dsReceive->setValue('warrantyID', $dbeItem->getValue('warrantyID'));
					$dsReceive->setValue('qtyOS', 1);
					$dsReceive->setValue('itemID', $dbeItem->getValue('itemID'));
					$dsReceive->setValue('partNo', $dbeItem->getValue('partNo'));
					$dsReceive->setValue('allowReceive', TRUE);
					$dsReceive->post();
				}
			}
			else{
				$sequenceNo++;
				$dsReceive->setUpdateModeInsert();
				$dsReceive->setValue('requireSerialNo', FALSE);
				$dsReceive->setValue('serialNo', '');
				$dsReceive->setValue('description', $dsPorline->getValue('itemDescription'));
				$dsReceive->setValue('sequenceNo', $sequenceNo);
				$dsReceive->setValue('orderSequenceNo', $dsPorline->getValue('sequenceNo'));
				$dsReceive->setValue('qtyOrdered', $dsPorline->getValue('qtyOrdered'));
				$dsReceive->setValue('qtyReceived', $dsPorline->getValue('qtyReceived'));
				$dsReceive->setValue('qtyToReceive', 0);
				$dsReceive->setValue('warrantyID', '');
				$dsReceive->setValue('qtyOS', $qtyOS);
				$dsReceive->setValue('itemID', $dbeItem->getValue('itemID'));
				$dsReceive->setValue('partNo', $dbeItem->getValue('partNo'));
				$dsReceive->setValue('allowReceive', TRUE);
				$dsReceive->post();
	...
	* and warranty info for adding a customeritem
	*/
	function getInitialStockReceieveQtys($customerID, &$dsPorline, &$dsReceive){
		...
		while ($dsPorline->fetchNext()){
			...
			*/
			if ( ($dbeItem->getValue('serialNoFlag') == 'Y') )
			{
				...
				$dbeCustomerItem->getRowsByCustomerAndItemID($customerID, $itemID);
				for ( $i=1 ; $i <= $qtyOS ; $i++ ){
					$sequenceNo++;
					$dsReceive->setUpdateModeInsert();
					$dsReceive->setValue('requireSerialNo', TRUE);
					...
					if ($dbeCustomerItem->fetchNext()) {
						$dsReceive->setValue('serialNo', $dbeCustomerItem->getValue('serialNo'));
						$dsReceive->setValue('warrantyID', $dbeCustomerItem->getValue('warrantyID'));
						$dsReceive->setValue('qtyOrdered', 1);
						$dsReceive->setValue('qtyReceived', 0);
						$dsReceive->setValue('qtyToReceive', 0);
						$dsReceive->setValue('qtyOS', 1);
						$dsReceive->setValue('allowReceive', TRUE);
						$dsReceive->setValue('customerItemID', $dbeCustomerItem->getPKValue());
						$dsReceive->post();
					}
					else{
						// all the rest are out of stock so include on one line and break our of for loop
						$dsReceive->setValue('serialNo', 'NOT IN STOCK');
						$dsReceive->setValue('customerItemID', null);		// indicates disabled
						$dsReceive->setValue('qtyOrdered', $qtyOS - $i + 1);
						$dsReceive->setValue('qtyOS', $qtyOS - $i + 1);   // remaining qty are o/s
						$dsReceive->setValue('warrantyID', null);
						$dsReceive->setValue('allowReceive', FALSE);
						$dsReceive->post();
						break;
					}
				}
	...
	function initialiseReceiveDataset(&$dsReceive)
	{
		...
		$dsReceive->addColumn('requireSerialNo', DA_INTEGER, DA_ALLOW_NULL);
		$dsReceive->addColumn('serialNo', DA_STRING, DA_ALLOW_NULL);
		$dsReceive->addColumn('description', DA_STRING, DA_ALLOW_NULL);
		$dsReceive->addColumn('sequenceNo', DA_INTEGER, DA_ALLOW_NULL);
		$dsReceive->addColumn('orderSequenceNo', DA_INTEGER, DA_ALLOW_NULL);
		$dsReceive->addColumn('qtyOrdered', DA_INTEGER, DA_ALLOW_NULL);
		$dsReceive->addColumn('qtyReceived', DA_INTEGER, DA_ALLOW_NULL);
		$dsReceive->addColumn('qtyToReceive', DA_INTEGER, DA_ALLOW_NULL);
		$dsReceive->addColumn('warrantyID', DA_INTEGER, DA_ALLOW_NULL);
		$dsReceive->addColumn('qtyOS', DA_INTEGER, DA_ALLOW_NULL);
		$dsReceive->addColumn('itemID', DA_ID, DA_ALLOW_NULL);
		$dsReceive->addColumn('customerItemID', DA_ID, DA_ALLOW_NULL);
		$dsReceive->addColumn('partNo', DA_INTEGER, DA_ALLOW_NULL);
		$dsReceive->addColumn('allowReceive', DA_INTEGER, DA_ALLOW_NULL);
	}
	function getAllWarranties(& $dsWarranty)
	{
		...
		$dbeWarranty->getRows();
		return ($this->getData($dbeWarranty, $dsWarranty));
	}
	...
	*/
	function receive($porheadID, $userID, & $dsGoodsIn){
		$this->setMethodName('receive');
		$this->getHeaderByID($porheadID, $dsPorhead);
		/*
		...
		*/
		if (
			...
			$this->receiveFromStock($porheadID, $dsPorhead, $userID, $dsGoodsIn);
		}
		else{
			$this->receiveFromNonStock($porheadID, $dsPorhead, $userID, $dsGoodsIn);
		}
	}
	/**
	* Receive selected items and qtys.
	...
	* @param Dataset $dsGoodsIn Dataset of recieved items
	*/
	function receiveFromNonStock($porheadID, &$dsPorhead, $userID, & $dsGoodsIn){
		...
		while ($dsGoodsIn->fetchNext()){
			...
			if ( $dsGoodsIn->getValue('requireSerialNo') ){
				...
				$dbeCustomerItem->setValue('itemID', $dsGoodsIn->getValue('itemID'));
				$dbeCustomerItem->setValue('userID', $userID);
				$dbeCustomerItem->setValue('custItemRef', $dsGoodsIn->getValue('description'));
				$dbeCustomerItem->setValue('contactID', $dsOrdhead->getValue('delContactID'));
				$dbeCustomerItem->setValue('despatchDate', date('Y-m-d'));
				$dbeCustomerItem->setValue('ordheadID', $dsPorhead->getValue('ordheadID'));
				$dbeCustomerItem->setValue('porheadID', $porheadID);
				...
				$dbeCustomerItem->setValue('custPORef', $dsOrdhead->getValue('custPORef'));
				...
				if ( ( $stockcat == 'M' ) or ( $stockcat == 'R' ) ){
					...
					$dbeCustomerItem->setValue('expiryDate', null);
				}
    ...
    @todo: update for many-to-many
        ...
        */
			...
			$dbePorline->setValue('porheadID', $porheadID);
			$dbePorline->setValue('sequenceNo', $dsGoodsIn->getValue('orderSequenceNo'));
			$dbePorline->getRow();
			$dbePorline->setValue('qtyReceived', $dbePorline->getValue('qtyReceived') + $dsGoodsIn->getValue('qtyToReceive'));
			$dbePorline->updateRow();
			...
			*/
			if ($dsOrdhead->getValue('customerID') == CONFIG_SALES_STOCK_CUSTOMERID){
				...
				$dbeItem->setValue('salesStockQty', $dbeItem->getValue('salesStockQty') + $dsGoodsIn->getValue('qtyToReceive'));
				$dbeItem->updateRow();
			}
			else if ($dsOrdhead->getValue('customerID') == CONFIG_MAINT_STOCK_CUSTOMERID){
				$dbeItem->getRow($dsGoodsIn->getValue('itemID'));
				$dbeItem->setValue('maintStockQty', $dbeItem->getValue('maintStockQty') + $dsGoodsIn->getValue('qtyToReceive'));
				$dbeItem->updateRow();
			}
		}//dsGoodsIn->fetchNext()
	}
	...
	* @param Dataset $dsGoodsIn Dataset of recieved items
	*/
	function receiveFromStock($porheadID, &$dsPorhead, $userID, & $dsGoodsIn){

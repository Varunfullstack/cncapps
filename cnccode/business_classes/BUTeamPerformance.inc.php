...
class BUTeamPerformance extends Business
{
    ...
    const PROJECT_TEAM_LEVEL        = 5;
    const SMALL_PROJECTS_TEAM_LEVEL = 3;
    const ESCALATION_TEAM_LEVEL     = 2;
    const HELPDESK_TEAM_LEVEL       = 1;
    ...
    */
    function update($year, $month)
    {
        ...
        $hdTeamTotal           = $this->getCount($year, $month, self::HELPDESK_TEAM_LEVEL);
        $hdTeamWithinSla       = $this->getCount($year, $month, self::HELPDESK_TEAM_LEVEL, true);
        $hdTeamFixAverageHours = $this->getFixAverageHours($year, $month, self::HELPDESK_TEAM_LEVEL);
        if ($hdTeamTotal > 0) {
            $hdTeamActualSlaPercentage = $hdTeamWithinSla / $hdTeamTotal * 100;
        } else {
            $hdTeamActualSlaPercentage = 100;
        }
        $esTeamTotal           = $this->getCount($year, $month, self::ESCALATION_TEAM_LEVEL);
        $esTeamWithinSla       = $this->getCount($year, $month, self::ESCALATION_TEAM_LEVEL, true);
        $esTeamFixAverageHours = $this->getFixAverageHours($year, $month, self::ESCALATION_TEAM_LEVEL);
        if ($esTeamTotal > 0) {
        ...
        $smallProjectsTeamTotal           = $this->getCount($year, $month, self::SMALL_PROJECTS_TEAM_LEVEL);
        $smallProjectsTeamWithinSla       = $this->getCount($year, $month, self::SMALL_PROJECTS_TEAM_LEVEL, true);
        $smallProjectsTeamFixAverageHours = $this->getFixAverageHours($year, $month, self::SMALL_PROJECTS_TEAM_LEVEL);
        ...
        $projectTeamTotal           = $this->getCount($year, $month, self::PROJECT_TEAM_LEVEL);
        $projectTeamWithinSla       = $this->getCount($year, $month, self::PROJECT_TEAM_LEVEL, true);
        $projectTeamFixAverageHours = $this->getFixAverageHours($year, $month, self::PROJECT_TEAM_LEVEL);
        ...
        $record = array(
            ...
            'hdTeamActualFixQtyPerMonth'      => $this->getFixCount($year, $month, self::HELPDESK_TEAM_LEVEL),
            ...
            'esTeamActualFixQtyPerMonth'      => $this->getFixCount($year, $month, self::ESCALATION_TEAM_LEVEL),
            ...
            'imTeamActualFixQtyPerMonth'      => $this->getFixCount($year, $month, self::SMALL_PROJECTS_TEAM_LEVEL),
            ...
            'projectTeamActualFixQtyPerMonth' => $this->getFixCount($year, $month, self::PROJECT_TEAM_LEVEL)
        );
    ...
    function getCount($year, $month, $teamLevel, $resolvedWithinSla = false)
    {
      ...
      WHERE
        pro_status = 'C'
        AND pro_custno <> " . CONFIG_INTERNAL_CUSTOMERID . " AND team.`level` = ?
        AND pro_priority < 5

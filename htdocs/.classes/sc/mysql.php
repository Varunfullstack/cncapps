...
class SC_MysqlConnection extends SC_Object
{
    ...
    function connect()
    {
        $this->connection = mysqli_connect($this->db_host, $this->user, $this->pass, $this->db_name);
        if (!$this->connection) {
...
class SC_MysqlStatement extends SC_Object
{
            ...
            if ($cnt > 0) {
                foreach ($this->value_binds as $ph => $pv) {
                    if (is_null($pv)) {
                        ...
                        $query = SC_String::strReplaceFirst(
                            SC_MYSQL_FIELD_VALUE_MARKER . "$ph",
                            "'" . mysqli_real_escape_string($this->connection, $pv) . "'",
                            $query
                        );
                    }
                }
    ...
    function foundRows()
    {
        $row = mysqli_fetch_row(@mysqli_query($this->connection, 'SELECT FOUND_ROWS()'));
        return $row[0];
    }

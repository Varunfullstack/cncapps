		...
		if ( $this->isUnique($field)  ) {
			if ( $this->getSelectStatement($field)  ) {						// we have a special statement defined for this field
				...
				if ($this->row[$this->getPKName()] != '') {
					$query_string .=
						"\r\nAND\r\n\t" . $this->getPKName() . " <> '" . mysql_escape_string( $this->getPKValue() )."'";
				}
				...
				$query_string =
							...
							"\r\nFROM\r\n\t" .	$this->table_name .
							"\r\nWHERE\r\n\tLOWER(" . $field . ") = " .
							"'".mysql_escape_string(strtolower($value)) ."'";
...
//			}
	...
	*/
	...
	function checkConnection() {
		// lazzy instantiation saves on resources
		SC_DB::checkGlobalConnection();
		
		if ( !$this->connection ){
			$this->connection = & $GLOBALS['connection'];
		}
	}
	function checkGlobalConnection() {
		if ( !isset($GLOBALS['connection'])	) {
			require_once(CONFIG_PATH_SC_CLASSES .		'mysql.php');
			$GLOBALS['connection'] 	=
				new SC_MysqlConnection(
					CONFIG_DB_USERNAME,
					CONFIG_DB_PASSWORD,
					CONFIG_DB_HOST,
					CONFIG_DB_NAME
            ...
            */
            if( strpos( $value , ',' ) !== false ){
              ...
              $statement .= ')';
						  ...
						  */
						  if ( $operator ){
							  ...
							  if ( SC_Date::isMySQLDate( $value ) ||	$this->isStringType( $field )	){
								  $statement .=  '"' . mysql_escape_string($value) . '"';
							  }
							  else{
								  $statement .=  mysql_escape_string($value);
							  }
                ...
                $statement .= ' LIKE "%' . mysql_escape_string($value) . '%"';
						  }

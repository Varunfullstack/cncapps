...
<form name="clientInformation" method="post" action="ClientInformation.php">
	<input type="hidden" name="contactID" value="<?php echo $_REQUEST['contactID']?>">
	<table width="45%" border="0" class="singleBorder">
		<tr> 
			<td width="17%" nowrap class="promptText"> Date of Meeting:</td>
			<td width="22%" > <input name="meetingDate" type="text" value="<?php echo $_REQUEST['meetingDate'];?>" size="50" maxlength="50"> 
			</td>
			<td width="3%" >&nbsp;</td>
			<td width="4%" class="promptText" >Time:</td>
			<td width="54%" >
				<input name="meetingTime" type="text" id="meetingTime" size="10" maxlength="10" value="<?php echo $_REQUEST['meetingTime'];?>">
		</td>
		</tr>
		<tr>
			...
			<td colspan="4">
			 <select					type="text"
				...
				<?php	while ($dbeUserList->fetchNext()){ ?>
					<option						<?php echo ($_REQUEST['attendeeUserID'] == $dbeUserList->getValue('userID'))?'SELECTED':'';?>
						value="<?php echo $dbeUserList->getValue('userID') ?>"
					>
					<?php echo $dbeUserList->getValue('firstName') . ' ' . $dbeUserList->getValue('lastName')?>
					</option>
				<?php } ?>
				</select>
			</td>
		...
		<tr> 
			...
			<td colspan="4"> <textarea name="meetingReason" cols="50" rows="2"><?php echo $_REQUEST['meetingReason'];?></textarea> 
			</td>
		</tr>
		<tr> 
			...
			<td colspan="4">
				...
				<input type="submit" name="Confirmation" value="Send Confirmation">

...
export default class MassAssignmentComponent extends MainComponent {
    ...
    getList=(items,value=null,onClick)=>{
        ...
        {items.map(item=><li onClick={()=>onClick(item)} className={"custom-list-item "+(item.id==value?" selected":"")} key={item.id}  >          
          ...
          <div>
            ...
            >
              ...
              <div className="  text-center">
                ...
                {this.state.filter.option.id==5?<CustomerSearch width={250} onChange={this.handleSelectCustomer}></CustomerSearch>:null}  
                </div>     
                <div
                  style={{ display: "flex", justifyContent: "space-between",padding:5 }}
                >
                    <ToolTip title="Move to left" width={30}>
                        <li className="fal fa-chevron-double-left fa-2x pointer" onClick={()=>this.handleMove(false)}></li>
                    </ToolTip>
                    <ToolTip title="Exchange" width={30}>
                        <li className="fal fa-exchange fa-2x pointer" onClick={()=>this.handleExchange()}></li>
                    </ToolTip>
                    <ToolTip title="Move to right"  width={30}>
                        <li className="fal fa-chevron-double-right fa-2x pointer"  onClick={()=>this.handleMove(true)}></li>
                    </ToolTip>
                 </div>
              </div>

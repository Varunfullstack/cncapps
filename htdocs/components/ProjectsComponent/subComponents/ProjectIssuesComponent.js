    ...
    const columns=[
     ...
     },
     {
      ...
      content:(issue)=>{
        ...
        <i className="fal  fa-edit color-gray2 pointer"  onClick={()=>this.setState({showModal:true,newIssue:false,currentIssue:issue})}></i>
      :null
      }
     },
     {
      ...
      content:(issue)=>{
        return issue.consID==this.props.currentUser.id?
        <i className="fal  fa-trash color-gray2 pointer"  onClick={()=>this.handleDeleteIssue(issue.id)}></i>
      :null
      }
     },
    ]
    ...
    <div key="content">
      <div className="form-group">
        ...
        <CNCCKEditor     
          ...
          onChange={(event) =>{
            currentIssue.issuesRaised= event.editor.getData();

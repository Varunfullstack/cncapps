...
class ItemsComponent extends MainComponent {
   ...
   scrollTimer;
          ...
          if(this.state.manufactureries.length==0)
          {
              ...
              Promise.all([
                ...
                this.apiManufacturer.getTypeList().then(res=>res.data)
                  ...
                  this.setState({itemTypes,warranties,renewalTypes,itemBillingCategories,manufactureries,showSpinner:false})
              })
            ...
            <table className="table">
                <tbody>                    
                    <tr>
                        ...
                        <td><input required   value={data.description} onChange={(event)=>this.setValue("description",event.target.value)} className="form-control"></input></td>
                        <td  className="text-right">Type</td>
                        <td><select className="form-control" value={data.itemTypeID } onChange={(event)=>this.setValue("itemTypeID",event.target.value)}>
                                <option></option>    
                                {itemTypes.map(item=><option key={item.id} value={item.id}>{item.description}</option> )}
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                        <select   value={data.manufacturerID||''} onChange={(event)=>this.setValue("manufacturerID",event.target.value)} className="form-control">
                                <option></option>
                                {manufactureries.map(w=> <option key={w.manufacturerID} value={w.id}>{w.name}</option>)}
                            </select>
                        </td>
                        <td  className="text-right">Warranty</td>
                        <td><select   value={data.warrantyID||''} onChange={(event)=>this.setValue("warrantyID",event.target.value)} className="form-control">
                                <option></option>
                    ...
                    <tr>
                        ...
                        <td><select value={data.renewalTypeID||''} onChange={(event)=>this.setValue("renewalTypeID",event.target.value)} className="form-control">
                                ...
                                {renewalTypes.map(w=> <option key={w.id} value={w.id}>{w.name}</option>)}
                            </select></td>
                    </tr>
                    <tr>
                        ...
                        <td><select  value={data.itemBillingCategoryID||''} onChange={(event)=>this.setValue("itemBillingCategoryID",event.target.value)}  className="form-control">
                                <option></option>
                                {itemBillingCategories.map(w=> <option key={w.id} value={w.id}>{w.name}</option>)}
                            </select></td>
                    </tr>
                    <tr>
                        <td className="text-right">Part Number	</td>
                        <td><input  value={data.partNo} onChange={(event)=>this.setValue("partNo",event.target.value)} className="form-control"></input></td>                        
                        <td className="text-right">Contract Response Time	</td>
                        <td><input  value={data.contractResponseTime} onChange={(event)=>this.setValue("contractResponseTime",event.target.value)}  className="form-control"></input></td>
                    </tr>
                    <tr>
                    <td  className="text-right">Old Part Number	</td>
                        <td><input  value={data.partNoOld} onChange={(event)=>this.setValue("partNoOld",event.target.value)} className="form-control"></input></td>                        
                        <td style={{verticalAlign:"top"}} className="childs text-right " colSpan={2} rowSpan={6}>{this.getChildItemsElement()}</td>

                    </tr>
                    <tr>
                        <td  className="text-right">Notes</td>
                        <td className="text-left" colSpan="1"><textarea  value={data.notes} onChange={(event)=>this.setValue("notes",event.target.value)}  className="form-control"></textarea></td>                         
                    </tr>
                    <tr>
    ...
    getChildItemsElement=()=>{
        ...
        return <div style={{color: "black",textAlign: "left",marginLeft: 10}}>
            ...
            <div className="flex-row">
                <ItemSearchComponent width={260} onSelect={this.handleChildSelect} value={childItem?.name}></ItemSearchComponent>
                <i className="fal fa-2x fa-plus color-white pointer icon ml-3" onClick={this.handleAddChild}></i>
            </div>
            {this.getChildItems()}
        </div> 
    }
    ...
    getChildItems=()=>{
        ...
        return <table className={this.getTableStyle()}>
            ...
            <tbody>
                ...
                <tr key={c.id}>
                    <td>{c.name}</td>
                    <td><input style={{width:30}} type="number" value={c.quantity}  onChange={(event)=>this.handleChildQuantity(c.id,event.target.value)}></input></td>
    ...
    getCheckBox=(name,yesNo=true)=>{
        ...
        return <input  checked={data[name]==trueValue} onChange={()=>this.setValue(name,data[name]==trueValue?falseValue:trueValue)}  type="checkbox"></input>
    }
    getModal=()=>{
        ...
        footer={<div key="footer">
            <button   onClick={this.handleClose} className="btn btn-secodary">Cancel</button>

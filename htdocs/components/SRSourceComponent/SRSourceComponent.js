...
class SRSourceComponent extends React.Component {
  ...
  getSearchElement(label, element, error = null, elementKey = null) {
    ...
    return el("div", { key: elementKey + "row", className: "row" }, [
      el(
        "div",
        { key: elementKey + "label", className: "col-1 promptText" },
        label
      ),
      el(
        "div",
        { key: elementKey + "element", style: { width: 250 } },
        element
      ),
  ...
  getSummary = (result) => {
    ...
    for (let i = 0; i < result.length; i++) {
      if (result[i].raiseType != null)
        summary[result[i].raiseType] = !summary[result[i].raiseType]
          ? 1
          : summary[result[i].raiseType] + 1;
      else summary["None"] = !summary["None"] ? 1 : summary["None"] + 1;
    }
        ...
        >
          <thead key="summaryHead">
            <tr key="summaryHeaderRow">
              {resultSummary.map((s, idx) => (
                <th key={idx}>{this.getIconElement(s[0])}</th>
              ))}
            </tr>
          </thead>
    ...
    switch (name) {
      case "Email":
        return el(ToolTip, {
          title: "Email",
          content: el("i", {
            className: `fal fa-envelope fa-2x icon pointer ${color}`,
          }),
        });
      case "Portal":
        return el(ToolTip, {
          title: "Portal",
          content: el("i", {
            className: `icon-chrome_icon fa-2x icon pointer ${color}`,
          }),
        });
      case "Phone":
        return el(ToolTip, {
          title: "Phone",
          content: el("i", {
            className: `fal fa-phone fa-2x icon pointer ${color}`,
          }),
        });
      case "On site":
        return el(ToolTip, {
          title: "On site",
          content: el("i", {
            className: `fal fa-building fa-2x icon pointer ${color}`,
          }),
        });
      case "Alert":
        return el(ToolTip, {
          title: "Alert",
          content: el("i", {
            className: `fal fa-bell fa-2x icon pointer ${color}`,
          }),
        });
      case "Sales":
        return el(ToolTip, {
          title: "Sales",
          content: el("i", {
            className: `fal fa-shopping-cart fa-2x icon pointer ${color}`,
          }),
        });
      case "Manual":
        return el(ToolTip, {
          title: "Manual",
          content: el("i", {
            className: `fal fa-user-edit fa-2x icon pointer ${color}`,
          }),
        });
      case "Total":
        return el(ToolTip, {
          title: "Total",
          content: el("i", {
            className: `fal fa-sigma fa-2x icon pointer ${color}`,
          }),
        });
      default:
        return el(ToolTip, {
          title: "Unknown",
          content: el("i", {
            className: `fal fa-question fa-2x icon pointer ${color}`,
          }),
        });
    }
  };
  ...
  getSearchResultElement() {
    const columns = [
        
      {
        ...
        icon: "fal fa-2x fa-inbox-in  color-gray2 pointer",
        className: "text-center",
        content: (sr) => this.getIconElement(sr.raiseType, " color-gray "),
      },
      {
        path: "CallReference",
        label: "",
        ...
        content: (sr) =>
          this.el(
            "a",
            { key: sr.inialActivity, href: sr.srLink, target: "_blank" },
            sr.CallReference
          ),
      },
      {
      ...
      },
      {
        ...
        content: (sr) => {
          ...
          if (
            (sr.status == "C" || sr.status == "F") &&
            sr.pro_contract_cuino == null
          )
            contractDisplay = "T&M";
          else if (
            (sr.status == "I" || sr.status == "P") &&
            sr.pro_contract_cuino == null
          ) {
            contractDisplay = "";
          }
          return this.el(
            "label",
            { key: "contractDisplay" + sr.CallReference },
            contractDisplay
          );
        },
      },

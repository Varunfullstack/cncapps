        ...
        const columns=[
            ...
            },
            {
             ...
             },
             {
                ...
                //icon: "fal fa-2x fa-file-alt color-gray2 pointer",
                sortable: true,
                content:(type)=><Toggle checked={type.includeInWeeklyReport} disabled={true}></Toggle>
             ...
             },
             {
                ...
                content:(type)=><ToolTip title="edit" >
                    <i className="fal fa-edit fa-2x pointer icon" onClick={()=>this.handleEdit(type)}></i>
                </ToolTip>
        ...
        return <Modal title="Project Type Name" show={showModal} width={500}
            ...
            content={<div  key="content">
                    <div className="form-group">
                        <label>Name</label>
                        <input required   value={data.name} onChange={(event)=>this.setValue("name",event.target.value)} type="text"></input>
                    </div>
                    
                    <div className="form-group">
                        <label>Notes</label>
                        <textarea required   value={data.notes} onChange={(event)=>this.setValue("notes",event.target.value)} type="text"></textarea>
                    </div>
                    <div className="form-group">
                        <label>Include In Weekly Report</label>
                        <Toggle checked={data.includeInWeeklyReport} onChange={()=>this.setValue("includeInWeeklyReport",!data.includeInWeeklyReport)}></Toggle>                        
                    </div>
                    </div>
        }

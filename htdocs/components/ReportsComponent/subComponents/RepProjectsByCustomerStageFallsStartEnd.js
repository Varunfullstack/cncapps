...
export default class RepProjectsByCustomerStageFallsStartEnd extends MainComponent { 
  api = new APIProjects(); 
  constructor(props) {
    super(props);
    this.state = {
  ...
  getData(){
    ...
    const {customerID,dateFrom,dateTo}=this.props;
    this.api.getProjectsByCustomerStageFallsStartEnd(customerID,dateFrom,dateTo)
    .then(projects=>{
  ...
  getProjectsTable=()=>{
    ...
    const columns=[
          ...
          },
        {
            ...
            //className: "text-center",
             content:(project)=><a style={{color:'black'}} href={`/Projects.php?action=edit&projectID=${project.projectID}`} target="_blank">{project.description}</a>
        }, 
        {

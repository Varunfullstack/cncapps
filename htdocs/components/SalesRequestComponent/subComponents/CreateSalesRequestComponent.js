    ...
    return (
      <div>
        ...
        <div className="form-group">
          ...
          <CustomerSearch customerName={data.customerName } onChange={(c)=>this.handleCustomerSelect(c)}></CustomerSearch>
        </div>
        <div className="form-group">
          <label>Template</label>
          <select value={data.type} style={{ width: 305 }} onChange={(event)=>this.handleTemplateSelect(event.target.value)}>
            <option></option>
            {options.map((o) => (
        ...
        <div className="form-group">
          <label>Files</label>
          <DragAndDropUploaderComponent onFilesChanged={(files, type) => this.handleFileSelected(files, type)}></DragAndDropUploaderComponent>
        </div>
        <div className="flex-row">
                {data.files.map(f=><div className="flex-row" key={f.name} style={{background: "#d1cece",    margin: 5,    padding: 5,   borderRadius: 10}}>
                    {f.name} 
                    <ToolTip title="Delete files" width={30}>
                <i className="fal fa-trash-alt color-red pointer ml-3" onClick={()=>this.handleDeleteFile(f)}></i>
                </ToolTip>
                </div>)}
        </div>
        <div className="flex-row">

    ...
    switch (A) {
        ...
        case 'Paste':
            try {
                ...
                if (FCKConfig.ForcePasteAsPlainText) FCK.PasteAsPlainText(); else FCKDialog.OpenDialog('FCKDialog_Paste', FCKLang.Paste, 'dialog/fck_paste.html', 400, 330, 'Security');
            }
            break;
        default:
...
FCKTools.DisableSelection = function (A) {
    if (FCKBrowserInfo.IsGecko) A.style.MozUserSelect = 'none'; else if (FCKBrowserInfo.IsSafari) A.style.KhtmlUserSelect = 'none'; else A.style.userSelect = 'none';
};
...
FCKXHtml._AppendNode = function (A, B) {
    ...
    switch (B.nodeType) {
        case 1:
            ...
            if (F) E = F(E, B, A); else E = this._AppendChildNodes(E, B, Boolean(FCKListsLib.NonEmptyBlockElements[D]));
            if (!E) return false;
            A.appendChild(E);
            break;
        case 3:
...
var FCKStyles = FCK.Styles = {
    _Callbacks: {}, _ObjectStyles: {}, ApplyStyle: function (A) {
    ...
    _UpdateElementInfo: function () {
        ...
        switch (A) {
            ...
            case 'list_contents':
                ...
                if (this.StartBlock && FCKConfig.EnterMode != 'br' && A == 'block_contents') this.SetStart(this.StartBlock, 1); else {
                    B = this._Range.startContainer;
                    if (B.nodeType == 1) {
    ...
    if (this.GetType() == 'Control') return FCKSelection.GetSelectedElement().parentNode; else {
        ...
        if (A) {
            if (A.anchorNode && A.anchorNode == A.focusNode) {
                var B = A.getRangeAt(0);
                if (B.collapsed || B.startContainer.nodeType == 3) return A.anchorNode.parentNode; else return A.anchorNode;
            }

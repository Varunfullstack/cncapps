...
<?php
	...
	foreach ($assoc_array as $index => $row){
		...
		<TABLE>
			<form
				action="<?php print SC_HTTP::serverVar('PHP_SELF') ?>?create_page_view=1"
				method="post"
			...
			<TR>
				...
				<TD>
					...
					$business = & $page_view;
					$field = 'page_view.name';
					$ajax_validation = false;
					$on_change = false;
					$value = $business->getValue($field);
					$on_key_down =false;
					$style = false;
...
//				$on_change = 'form.submit()';
		foreach ( $_SESSION[$display_fields_tag] as $field) {
		?>
			<TH class="filter" nowrap align="center" >
					<?php
/*
...
?>
	...
	<TR>
		...
		foreach ($_SESSION[$display_fields_tag] as $field) {
			...
			?>
			<TD
				width="<?php print $business->getPercentageWidth( $field, SC_HTTP::sessionVar( $display_fields_tag ), $additional_width) ?>%"
				align="left"
				valign="top"
							...
							*/
							if ( $already_ordered_by ) {
								...
								?>
									...
									<img class="icon" id="sort_image_<?php print $field?>" src="images/up.gif" width="10" height="10" border="0">
								<?php
								}
								else{
								?>
									<!-- ascending order image -->
									<img class="icon"  id="sort_image_<?php print $field?>" src="images/down.gif" width="10" height="10" border="0">
								<?php
								}
							}
							else{
								?>
								<img class="icon" id="sort_image_<?php print $field?>" src="images/spacer_icon.gif" width="10" height="10" border="0"/>
								<?php
						...
						<TD align="right">
							...
							if ( SC_HTTP::javascriptIsEnabled() && SC_HTTP::sessionVar( $show_fields_tag ) ){
								...
								<input
									...
									<?php print in_array( $field, SC_HTTP::sessionVar( $display_fields_tag ) ) ? SC_HTML_CHECKED : '' ?>
									value="1"
								>
								<?php
							}
...
<!-- Start rows -->
	...
	while ( $row = $results->fetchAssoc() ){
		...
		<tr
			...
			?>
			onMouseOver="this.bgColor='#FFFFCC';" onMouseOut="this.bgColor='';"
		>			
		<?php
		...
		?>
			...
			switch ( $business->getType( $field ) ) {
				...
				case SC_DB_FILE_NAME:
					$name				= $row[$business->getPKName()] . '_' . $row[$business->getTableFieldName('file_name')];
...
if ( SC_HTTP::sessionVar( $show_field_chooser_tag ) ) {
	?>
		...
		>
		<TR>
			<TH colspan="4" class="column_heading">
				<a <?php print SC_Ajax::renderLink( SC_HTTP::phpSelf() . '?show_field_chooser=0' ) ?>>Hide Field Chooser</a>
			</TH>
		</TR>
		<TR>
		...
		?>
			<TR onMouseOver="this.bgColor='#FFFFCC';" onMouseOut="this.bgColor='';">
				<TD nowrap class="formLabel" width = "50%">
					<?php
//					if ( $section_cache = $cache->sectionStart('label_' . $field , 24) ) {
						print SC_String::display( $business->getLabel( $field ), SC_STRING_FMT_UC_WORDS );
//						$cache->sectionEnd( $section_cache );
				...
				<TD width = "5%">
					...
					if ( $field == str_replace(' DESC', '' , SC_HTTP::sessionVar( $order_by_tag ) ) ) {
						...
						if ( strpos( $order_by , 'DESC' ) ){
							?>
							<!-- ascending order image -->
							<img class="icon" src="images/down.gif" width="10" height="10" border="0">
							<?php
						}
						else{
							?>
							<!-- descending order image -->
							<img class="icon" src="images/up.gif" width="10" height="10" border="0">
							<?php
...
<?php
	...
	?>
		<TR>
			<TH colspan="4" class="column_heading">
				<a <?php print SC_Ajax::renderLink( SC_HTTP::phpSelf() . '?show_field_chooser=1' ) ?>>Show Field Chooser</a>
			</TH>
		</TR>
<?php
}
